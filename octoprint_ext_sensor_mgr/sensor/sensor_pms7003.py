...
class Pms7003(Sensor):
    ...
    def config_params(cls):
        if not cls._is_config_params_init:
            cls._config_params = copy.deepcopy(super(Pms7003, cls).config_params())
            ...
            cls._config_params['serial_dev'] = ConfigProperty(data_type=str, value_list=serial_dev_list(), default_value=None, label='Serial Device (/dev/ttySx)')
    ...
    def output_config(self) -> dict():
        ...
        if SensorOutputType.PM2_5 in self.output_type:
            config['pm2_5'] = dict(type=SensorOutputType.PM2_5.name, unit='ug/m3')
        if SensorOutputType.PM1_0 in self.output_type:
            config['pm1_0'] = dict(type=SensorOutputType.PM1_0.name, unit='ug/m3')
        if SensorOutputType.PM10 in self.output_type:
            config['pm10'] = dict(type=SensorOutputType.PM10.name, unit='ug/m3')
